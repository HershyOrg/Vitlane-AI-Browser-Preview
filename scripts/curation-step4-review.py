#!/usr/bin/env python3
"""Local axis review: real Managed AI, Korean/Shopify/Amazon saved fixtures.

Use a dedicated database. This launcher never resets a database and never
prints credentials. Use --build-web to align the review UI with the AI mode.
"""
import argparse
import os
...
parser.add_argument("--database-name", default="vitlane_curation_step4")
parser.add_argument("--port", default="18086")
parser.add_argument("--stub-ai", action="store_true", help="Use the deterministic model fixture for repeatable tests")
parser.add_argument("--build-web", action="store_true", help="Build the review UI with labels matching the selected AI mode")
...
if args.build_web:
    ...
    web_environment.update(
        VITE_ALLOW_DEV_AUTH_UI="true", VITE_RESEARCH_REVIEW_STUB="true",
        VITE_RESEARCH_REVIEW_LIVE_AI="false" if args.stub_ai else "true",
    )
    # Vite needs presentation flags only, never the Managed provider credential.
    web_environment.pop("MANAGED_OPENAI_API_SECRET", None)
    subprocess.run(["npm", "run", "build"], cwd=root / "web", env=web_environment, check=True)
...
os.environ.update(
    ...
    SHOPIFY_UCP_ENABLED="false", KOREAN_PRODUCT_SEARCH_ENABLED="true",
    ...
    # Review adapters retain the real per-source single-call admission guard.
    MANAGED_RUNNER_CONCURRENCY="1",
...
print(f"Step 4 review: http://127.0.0.1:{args.port} — AI {'stub' if args.stub_ai else 'live'}; KR/Shopify/Amazon saved fixtures", flush=True)
