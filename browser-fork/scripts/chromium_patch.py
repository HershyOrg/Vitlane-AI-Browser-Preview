...
PIN = json.loads((ROOT / "chromium/revision.json").read_text(encoding="utf-8"))
...
PLANNER = ROOT / "chromium/java/org/chromium/chrome/browser/lane/LaneOpenAiPlanner.java"
...
def make_changes(original):
    ...
    edit(SOURCES, "chrome_java_sources = [\n", "chrome_java_sources = [\n"
         ...
         '  "java/src/org/chromium/chrome/browser/lane/LaneOpenAiPlanner.java",\n'
...
""")
    ...
    changed[destination + "LaneOpenAiPlanner.java"] = checked_source(PLANNER, [
        "https://api.openai.com/v1/responses", "actionFromResponse", "buildRequest",
    ])
    encoded = base64.b64encode(page_agent.encode()).decode()
    chunks = [encoded[i:i + 100] for i in range(0, len(encoded), 100)]
    # Keep every class-file UTF-8 constant below Java's 65,535-byte limit.
    # String concatenation would fold all chunks into a single oversized constant.
    literal = ('new StringBuilder(' + str(len(encoded)) + ')\n            '
               + '\n            '.join('.append(' + json.dumps(c) + ')' for c in chunks)
               + '\n            .toString()')
...
def main():
    ...
    if args.check:
        if not output.exists() or output.read_text(encoding="utf-8") != patch:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        for path, expected in changed.items():
            if (work / path).read_text(encoding="utf-8") != expected:
